            ...
            <div className="bidders-section">
              <div className="section-header">
                ...
                <span className="bidders-count">
                  {displayAuction.auction_bidders?.length || displayAuction.InvitedBidders?.split(', ').length || 0} bidders invited
                </span>
              </div>
            ...
            <div className="rankings-section">
              ...
              {rankings.length > 0 ? (
                    ...
                    <div className="winner-highlight">
                      ...
                      <div className="winner-info">
                        <div className="winner-details">
                          <span className="winner-name">{rankings[0].name}</span>
                          <span className="winner-company">{rankings[0].company}</span>
                        </div>
                        <div className="winner-bid">
                          <span className="winning-amount">{formatCurrency(rankings[0].amount)}</span>
                          <span className="bid-time">
                            Bid Time: {new Date(rankings[0].bid_time).toLocaleString('en-GB')}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <table className="rankings-table">
                      ...
                      <tbody>
                        {rankings.map((bidder, index) => (
                          <tr key={bidder.bidder_id} className={index === 0 ? 'winner-row' : ''}>
                            ...
                            <td className="bid-time">
                              {new Date(bidder.bid_time).toLocaleString('en-GB')}
                            </td>
                            <td>
                              <span className={`status-badge ${index === 0 ? 'status-winner' : 'status-participated'}`}>
                    ...
                    <div className="stats-grid">
                      ...
                      <div className="stat-item">
                        ...
                        <span className="winning-amount">
                          {rankings.length > 0 ? formatCurrency(rankings[0].amount) : 'No bids'}
                        </span>
                      </div>
                      <div className="stat-item">
                        ...
                        <span>
                          {rankings.length > 0 
                            ? formatCurrency(Math.max(...rankings.map(r => r.amount)))
                            : 'No bids'
                          }
                        </span>
                      </div>
                      <div className="stat-item">
                        ...
                        <span>
                          {rankings.length > 0 
                            ? formatCurrency(rankings.reduce((sum, r) => sum + r.amount, 0) / rankings.length)
                            : 'No bids'
                          }
                        </span>
                      </div>
                ...
                <div className="no-bidding-data">
                  ...
                  <p>
                    {(displayAuction.calculated_status || displayAuction.status) === 'pending' 
                      ? 'Auction has not started yet'
                      : (displayAuction.calculated_status || displayAuction.status) === 'live'
                      ? 'Auction is currently live - refresh to see latest bids'
                      : 'No bids were placed in this auction'
                    }
                  </p>
                  {(displayAuction.calculated_status || displayAuction.status) === 'live' && (
                    <button className="btn btn-refresh" onClick={fetchAuctionDetails}>
                      Refresh Data
                    </button>
                  )}
                </div>
